...
class ChatRepository:
    ...
    def get_user_chat_rooms(self, user_id: str, user_type: str, include_archived: bool = False) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            # Convert raw MongoDB documents to ChatRoom objects and serialize
            serialized_rooms = []
                ...
                try:
                    ...
                    for key, value in room_data.items():
                        if hasattr(value, '__class__') and 'ObjectId' in str(type(value)):
                            room_data[key] = str(value)
                    
                    # Create ChatRoom object and serialize to dict
                    room_obj = ChatRoom.from_dict(room_data)
                    serialized_rooms.append(room_obj.to_dict())
                except Exception as e:
                    logger.error(f"Error converting room {room_data.get('_id', 'unknown')}: {str(e)}")
                    continue
